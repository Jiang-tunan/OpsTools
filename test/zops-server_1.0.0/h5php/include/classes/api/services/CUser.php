...
class CUser extends CApiService {
	public const ACCESS_RULES = [
		...
		'checkauthentication' => [],
        'disableuser' => ['min_user_type' => USER_TYPE_ZABBIX_USER],
		'login' => [],
        ...
        'uploadimg' => ['min_user_type' => USER_TYPE_ZABBIX_USER]
	];
	...
	public function get($options = []) {
		...
		if ($options['usrgrpids'] !== null) {
            ...
            if ($options['enable'] === true) {
                $sqlParts['where'][] = 'ug.userid not in (select userid from users_groups where userid = u.userid and usrgrpid = 9) and ug.usrgrpid in ('.implode(',',$options['usrgrpids']).')';
            }
            else if($options['enable'] === false) {
                //如果需要筛选指定用户组id，而且为禁用的用户
                $sqlParts['where'][] =  'ug.userid in (select userid from users_groups where userid = u.userid and usrgrpid in ('.implode(',',$options['usrgrpids']).')) and ug.usrgrpid = 9';
            }else{
                $sqlParts['where'][] = $options['excludeSearch'] ? 'ug.usrgrpid!='.implode(',',$options['usrgrpids']) : dbConditionInt('ug.usrgrpid', $options['usrgrpids']);
            }
		}else{
            if ($options['enable'] === true) {
                $sqlParts['from']['users_groups'] = 'users_groups ug';
                $sqlParts['where']['uug'] = 'u.userid=ug.userid';
                $sqlParts['where'][] =  'ug.usrgrpid not in (select usrgrpid from users_groups where userid = u.userid and usrgrpid = 9)';//查询用户组不为9的用户
            }
            else if($options['enable'] === false) {
                $sqlParts['from']['users_groups'] = 'users_groups ug';
                $sqlParts['where']['uug'] = 'u.userid=ug.userid';
                $sqlParts['where'][] = 'ug.usrgrpid = 9';
            }
        }

        if ($options['roleids'] !== null) {
            zbx_value2array($options['roleids']);

            $sqlParts['where'][] = dbConditionInt('u.roleid', $options['roleids']);
        }
		...
		while ($user = DBfetch($res)) {
			...
			if ($options['countOutput']) {
                ...
                $result[$user['userid']]['total'] = API::getTotal('user',$options);
			}
		}
    ...
    public function uploadimg($options = []){
        if($options['basecode'] == ''){
            self::exception(ZBX_API_ERROR_PARAMETERS, '参数错误,没有上传图片');
        }

        if($url = $this->base64toimg($options['basecode'])){
            return $url;
        }else{
            self::exception(ZBX_API_ERROR_PARAMETERS, '上传错误,base64格式不正确');
        }
    }
	/**
	 * @param array $users
	...
	public function create(array $users) {
        ...
        if($users['enable'] === true){
            if(count($users['usrgrps']) == 0) {
                $users['usrgrps'] = [['usrgrpid' => 8]];
            }
        }else if($users['enable'] === false){
            $users['usrgrps'] = [['usrgrpid' => 9]];
        }
	...
	private function validateCreate(array &$users) {
		...
		$api_input_rules = ['type' => API_OBJECTS, 'flags' => API_NOT_EMPTY | API_NORMALIZE, 'uniq' => [['username']], 'fields' => [
            ...
            'email' =>	['type' => API_STRING_UTF8],
            'phone' =>	['type' => API_STRING_UTF8],
            'ding' =>	['type' => API_STRING_UTF8],
            'wechat' =>	['type' => API_STRING_UTF8],
            'memo' =>	['type' => API_STRING_UTF8],
            'avatar' =>	['type' => API_STRING_UTF8],
            'department_id' =>	['type' => API_ID],
	...
	public function update(array $users) {
        if(isset($users['passwd'])){
            $user = $this->get(["userids"=>$users['userid']]);

            if(!$user || !isset($user[0])){
                self::exception(ZBX_API_ERROR_PARAMETERS, '该用户不存在');
            }
            ...
            $user_data = $this->findAccessibleUser($user[0]['username'], $sensitive, $default_auth_type, true);

            if(!isset($users['current_passwd']) || !password_verify($users['current_passwd'], $user_data['db_user']['passwd'])){
                self::exception(ZBX_API_ERROR_PARAMETERS, '原密码不正确');
            }
        }

        $users = $this->disablecheck($users);
    ...
    private function disablecheck($users){
        $userArr = $this->get([
        ...
        foreach($userArr[$users['userid']]['usrgrps'] as $v){
            if($v['usrgrpid'] != 9){
                array_push($order,['usrgrpid'=>$v['usrgrpid']]);
            }
        }

        if($users['enable'] === true){
            $order = count($users['usrgrps']) > 0 ? $users['usrgrps'] : $order;//如果有选择新的用户组，新用户组会覆盖原来用户组，如果没有选择，使用原来的非禁用用户组

            //如果原来的用户组只有禁用用户组，也没有选择新的用户组，这时候默认分配到访客用户组8
            if(count($order) > 0){
                $users['usrgrps'] = $order;
            }else{
                $users['usrgrps'] = [['usrgrpid'=>8]];
    ...
    public function disableuser(array $users){
        $users = $this->disablecheck($users);

        $this->validateUpdate($users, $db_users);

        self::updateForce($users,$db_users);
        echo json_encode(["jsonrpc"=>"2.0","result"=>["userids"=>array_column($users,'userid')],"id"=>1]);
    }
	...
	public function login(array $user) {
        ...
        $role = API::Role()->get(['roleids'=>$db_user['roleid'],'selectRules'=>['front'],'output'=>['roleid']]);

        if(isset($role[0]['rules']['front'])){
            $db_user['rules'] = $role[0]['rules']['front'];
        }else{
            $db_user['rules'] = [];
        }
	...
	public function checkAuthentication(array $session): array {
		...
		$db_users = DB::select('users', [
			'output' => ['userid', 'username', 'name', 'surname', 'url', 'autologin', 'autologout', 'lang', 'refresh',
				...
				'userdirectoryid', 'ts_provisioned','avatar'
			],
			'userids' => $userid
		]);
        ...
        if(isset($session['isfront'])){
            $roleclass = new CRole();
            $role = $roleclass->get(['roleids'=>$db_user['roleid'],'selectRules'=>['front'],'output'=>['roleid'],'isfront'=>$session['isfront']]);
	...
	protected function addRelatedObjects(array $options, array $result) {
        ...
        $departmentIds = zbx_objectValues($result, 'department_id');

        if(count($departmentIds) > 0){
            $db_department = DBselect(
                'SELECT id,name'.
                ' FROM department'.
            ...
            foreach ($result as $userid => $user) {
                ...
                $groupArr = array_column($result[$userid]['usrgrps'],'usrgrpid');
                $result[$userid]['status'] = in_array(9,$groupArr) ? false : true;
            }

            while ($db_department_value = DBfetch($db_department)) {
                $result[$userid]['department'] = $db_department_value;
            }
        }
