...
//        echo
    ...
    public function getData(){
        ...
        foreach($this->items as $key => $value){
            $data[$value['itemid']] = $maindatearr =  [];
            $maindate = '';
            foreach($this->timex as $timekey => $time){
                $timearr = explode('-',$time);
                if(strlen($time) == 10 || count($timearr) > 2){
                    $maindate = strlen($time) == 10 ? date('Y-m-d',$time) : $time;
                }

                if(count($timearr) > 2){
                    $data[$value['itemid']][$timekey] = ['clock' => strtotime($time),'data'=>0,'count'=>0,'itemid'=>$value['itemid']];
                }elseif(count($timearr) == 2){
                    $maindatearr = $timearr;
                    $data[$value['itemid']][$timekey] = ['clock' => strtotime($maindatearr[0] . '-' .$time),'data'=>0,'count'=>0,'itemid'=>$value['itemid']];
                }elseif(strlen($time) == 10){
                    $data[$value['itemid']][$timekey] = ['clock' => $time,'data'=>0,'count'=>0,'itemid'=>$value['itemid']];
                }else{
                    $time = strlen($time) == 8 ? $maindate . ' ' .$time : $maindate . ' ' .$time.':00';
                    $data[$value['itemid']][$timekey] = ['clock' => strtotime($time),'data'=>0,'count'=>0,'itemid'=>$value['itemid']];
                }
            }
        }
       ...
       // print_r($this->data);
        ...
        foreach($this->dataResult as $key => $value){
            foreach($value['avg'] as $key2 => $dataResultValue){
                foreach($data[$key] as $datakey => &$datavalue){
                   $starttime = $datavalue['clock'];
                   $endtime = isset($data[$key][$datakey + 1]) ? $data[$key][$datakey + 1]['clock'] : 10000000000;

                   $clockarr = explode('.',$value['clock'][$key2]);

//                   echo 1;
//                    if($key2 == 1809 && $key == 45593){
//                        echo '   ';
//                        echo $datavalue['data'];
//                        echo '   ';
//                        echo $datavalue['count'];
//                        echo '   ';
//                    }

                   if( count($clockarr) == 1 && $clockarr[0] >= $starttime && $clockarr[0] < $endtime){

                       $valuearr = explode('.',$value['avg'][$key2]);

                       if(count($valuearr) == 1){
                           $datavalue['data'] += $value['avg'][$key2];
                           $datavalue['count'] ++;
                       }
        ...
        foreach($data as $key => $value){
            foreach($value as $key2 => $value2){
                $data[$key][$key2]['data'] = $value2['count'] != 0? round($data[$key][$key2]['data'] / $value2['count'],2) : 0;
                $data[$key][$key2]['data'] = round($data[$key][$key2]['data']  / 1024 / 1024,2);
            }
        ...
        $dateStr = $this->period >= 184755  ? 'm-d H:i' : 'H:i';


        foreach($this->timex as $key => &$value){
            if(strlen($value) == 10 && $value < $this->stime){
                $value = $this->stime;
            }

            if(strlen($value) == 10){
                $value = date($dateStr,$value);
            }
        }

        //print_r($data);
        return ['timex'=>$this->timex,'data'=>$data];
    }
		...
		for ($i = 0, $i_max = count($mains) - 2; $i <= $i_max; $i++) {
			...
			foreach ($this->getDateTimeIntervals($mains[$i], $mains[$i + 1], $optimal['intervals']['sub']) as $time) {
				...
				if ($pos >= max($pos_min, $label_size) && $pos <= min($pos_max, $this->period - $label_size)) {
                    $this->timex[] = date($optimal['format']['sub'], $time);
