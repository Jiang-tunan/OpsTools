...
class CHostGroup extends CApiService {
	...
	public function get(array $options) {
        ...
        $sqlParts['select']['hosts_groups'] = 'hg.hostid';
        ...
        if(isset($options['hostids'])){
            array_push($sqlParts['select'],'hg.hostid');
            array_push($sqlParts['select'],'g.image');
        }
		...
		while ($group = DBfetch($res)) {
			if ($options['countOutput']) {
                ...
                $group['key'] = 'hostgroup'. $group['groupid'];
		...
		if ($options['selectHosts'] !== null) {
			if ($options['selectHosts'] !== API_OUTPUT_COUNT) {
                ...
                foreach($result as $k => &$v){
                    foreach($v['hosts'] as &$vv){
                        $vv['key'] = 'hostgroup'. $v['groupid'] . $vv['hostid'];
                    }
                    if(empty($v['hosts'])) unset($result[$k]);
                }
                sort($result);
			}
			else {
