...
abstract class CHostGeneral extends CHostBase {
	...
	public const ACCESS_RULES = [
        ...
        'exportinventory' => ['min_user_type' => USER_TYPE_ZABBIX_USER],
        'discoverupdate' => ['min_user_type' => USER_TYPE_ZABBIX_USER]
	];
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		if ($options['selectItems'] !== null || $options['getHistory']) {
			if ($options['selectItems'] != API_OUTPUT_COUNT || $options['getHistory']) {
				$items = API::Item()->get([
					'output' => $options['getHistory'] ? ['hostid','itemid','key_','name'] : $this->outputExtend($options['selectItems'], ['hostid', 'itemid']),
					'hostids' => $hostids,
					'nopermissions' => true,
					'preservekeys' => true,
                    'selectTags' => 'extend'
                ...
                if($options['getHistory']){
                    ...
                    $historyKeys = [
//                        'vfs.fs.total[hrStorageSize.31]'=>['history'=>3,'key'=>'storage_total','type'=>1],//总存储
//                        'vfs.fs.used[hrStorageUsed.31]'=>['history'=>3,'key'=>'storage_used','type'=>1],//使用存储
                        'vm.memory.total[memTotalReal.0]'=>['history'=>3,'key'=>'memory_total','type'=>1],//总内存
                        'agent.ping'=>['history'=>3,'key'=>'ping_status','type'=>0],
                        'icmpping[]'=>['history'=>3,'key'=>'ping_status','type'=>0],
//                        'vm.memory.available[snmp]'=>['history'=>3,'key'=>'memory_free','type'=>1],//可用内存
//                        'system.cpu.util[snmp,0]'=>['history'=>0,'key'=>'cpu_util','type'=>0],//CPU使用率
                        'system.cpu.num[snmp]'=>['history'=>3,'key'=>'cpu_num','type'=>0],//CPU核数
                        'system.hw.uptime[hrSystemUptime.0]'=>['history'=>3,'key'=>'uptime','type'=>2]//硬件在线时间
                    ];
                    ...
                    foreach($items as $v){
                        //统计端口数和网卡
                        if(!isset($network_cardArr[$v['hostid']])){
                            $network_cardArr[$v['hostid']] = ['interface'=>[],'description'=>[]];
                        }

                        foreach($v['tags'] as $vv){
                            if($vv['tag'] == 'interface' || $vv['tag'] == 'description'){
                                if(!isset($network_cardArr[$v['hostid']][$vv['tag']][$vv['value']])){
                                    $network_cardArr[$v['hostid']][$vv['tag']][$vv['value']] = 0;
                                }

                                $network_cardArr[$v['hostid']][$vv['tag']][$vv['value']] ++;
                            }
                        }

                        //匹配硬件CPU，内存等数值
                        if(array_key_exists($v['key_'],$historyKeys)){
                            $history = API::History()->get(['itemids'=>$v['itemid'],'history'=>$historyKeys[$v['key_']]['history'],'limit'=>1,'sortorder'=>'DESC','sortfield'=>'clock']);
                            $val = isset($history[0]) ? $history[0]['value'] : 0;
                            switch ($historyKeys[$v['key_']]['type']){
                                case 1:
                                    $val = round($val / 1024 / 1024 / 1024,2);//存储和内存返回的数据精确到gb
                                    break;
                                case 2:
                                    $val = round($val / 86400,2);//硬件在线时间返回的秒数精确到天
                                    break;
                                default:break;
                            }
                            $result[$v['hostid']][$historyKeys[$v['key_']]['key']] = $val;
                        }
                    }

                    foreach($result as &$v){
                        $v['network_card'] = count($network_cardArr[$v['hostid']]['descrition']);
                        $v['ports_num'] = count($network_cardArr[$v['hostid']]['interface']);

                        //$v['cpu_util'] = round($v['cpu_util'] ,2);
                        //$v['memory_used'] = round($v['memory_total'] - $v['memory_free'],2);
//                        $v['storage_util'] = $v['storage_total'] == 0 ? 0 : round($v['storage_used'] / $v['storage_total'] * 100,2) ;
//                        $v['memory_util'] = $v['memory_total'] == 0 ? 0 : round($v['memory_used'] / $v['memory_total'] * 100,2) ;
                        unset($v['items']);
                    }
                }else{
