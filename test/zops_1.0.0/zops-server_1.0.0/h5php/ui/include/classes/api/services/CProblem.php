...
class CProblem extends CApiService {
	...
	protected $sortColumns = ['eventid','clock','severity','r_clock'];
	...
	public function get($options = []) {
        ...
        if($options['isclosing'] == 2){
            $sqlParts['from']['a'] = 'acknowledges a';
            $sqlParts['where'][] = 'p.eventid in (select eventid from acknowledges where action % 2 = 1 and eventid = p.eventid) 
            or p.eventid not in (select eventid from acknowledges where eventid = p.eventid) and p.r_eventid is not null';
        }else if($options['isclosing'] == 1){
            $sqlParts['where'][] = 'p.eventid not in (select eventid from acknowledges where action % 2 = 1 and eventid = p.eventid) and p.r_eventid is null
            or p.eventid in (select eventid from acknowledges where eventid = p.eventid and action % 2 != 1) and p.r_eventid is not null';
        }
...
//		}
        ...
        $sqlParts['select']['items'] = 'i.hostid';
		$res = DBselect(self::createSelectQueryFromParts($sqlParts), $sqlParts['limit'],$options['current'] ? ($options['current'] - 1) * $sqlParts['limit'] : 0);
		while ($event = DBfetch($res)) {
			if ($options['countOutput']) {
                ...
                $result[$event['eventid']]['time_str'] = zbx_date2str(DATE_TIME_FORMAT_SECONDS, $event['clock']);

                $result[$event['eventid']]['total'] =API::getTotal('problem',$options);

                $result[$event['eventid']]['recovery_time_str'] = ($event['r_eventid'] != 0) ? zbx_date2str(DATE_TIME_FORMAT_SECONDS,  $event['r_clock']) : '';
                $result[$event['eventid']]['during_time_str'] = ($event['r_eventid'] != 0)
                    ? zbx_date2age($event['clock'], $event['r_clock'])
                    : zbx_date2age($event['clock']);
                $result[$event['eventid']]['acknowledged'] = ($event['acknowledged'] == EVENT_ACKNOWLEDGED) ? true : false;
                $result[$event['eventid']]['ack_str'] = ($event['acknowledged'] == EVENT_ACKNOWLEDGED) ? _('Yes') : _('No');
                $result[$event['eventid']]['delay'] = $event['alert_delay'] > 0 ? true : false;
			}
		}
        ...
        $triggerid = array_column($result,'objectid');
        $userid = [];

        foreach($result as $k => $v){
            foreach($v['acknowledges'] as $vv){
                array_push($userid,$vv['userid']);
            }
        }
        ...
        $triggers_hosts_list = $house = $interface = [];
        $triggers = API::Trigger()->get($options);

        foreach (getTriggersHostsList($triggers) as $triggerid => $trigger_hosts) {
            //返回机房信息和设备ip
            $housetemp = zbx_objectValues($trigger_hosts, 'house');
            $interfacetemp = zbx_objectValues($trigger_hosts, 'interfaces');


            $house[$triggerid] = [];
            $interface[$triggerid] = [];

            if(isset($housetemp[0])){
                $house[$triggerid] = $housetemp[0];
            }

            if(isset($interfacetemp[0])){
                $interface[$triggerid] = $interfacetemp[0];
            }
            $triggers_hosts_list[$triggerid] = implode(', ', zbx_objectValues($trigger_hosts, 'name'));
        }

        foreach ($result as $k => &$v){
            if(isset($v['acknowledges'])){
                $v['status_str'] =  getEventStatusString(hasEventCloseAction($v['acknowledges']), $v);

                foreach($v['acknowledges'] as $kk=>$vv){
                    $result[$k]['acknowledges'][$kk]['time_str'] = zbx_date2str(DATE_TIME_FORMAT_SECONDS, $event['clock']);
                    foreach($users as $vvv){
                        if($vvv['userid'] == $vv['userid']){
                            $result[$k]['acknowledges'][$kk]['username'] = $vvv['username'];
                        }
                    }
                }
            }

            $v['host'] = $triggers_hosts_list[$v['objectid']];
            $v['house'] = $house[$v['objectid']];
            $v['ip'] = isset($interface[$v['objectid']][0]) ? $interface[$v['objectid']][0]['ip'] : '';
        }
    ...
    public function update(array $problems): array {
        $this->validateUpdate($problems, $db_problem);

        $upd_problems = [];

        foreach ($problems as $problem) {
            $upd_problem = DB::getUpdatedValues('problem', $problem, $db_problem[$problem['eventids']]);
            $upd_ack  = DB::getUpdatedValues('acknowledges', $problem, $db_problem[$problem['eventids']]);

            if ($upd_problem) {
                $upd_problems[] = [
                    'values' => $upd_problem,
                    'where' => ['eventid' => $problem['eventids']]
                ];
            }
        ...
        if ($upd_problems) {
            DB::update('problem', $upd_problems);
        }

        if($upd_acks){
            DB::update('acknowledges',$upd_acks);
        }

        return ['eventids' => array_column($problems, 'eventids')];
    }

    private function validateUpdate(array &$problems,?array &$db_problems): void {
        $api_input_rules = ['type' => API_OBJECTS, 'flags' => API_NOT_EMPTY | API_NORMALIZE, 'uniq' => [['name']], 'fields' => [
            'eventids' =>			['type' => API_ID, 'flags' => API_REQUIRED],
            'action'=>          ['type' => API_INT32],
            'alert_delay'=>          ['type' => API_INT32],
            'message' =>			['type' => API_STRING_UTF8],
        ]];

        if (!CApiInputValidator::validate($api_input_rules, $problems, '/', $error)) {
            self::exception(ZBX_API_ERROR_PARAMETERS, $error);
        }

        $db_problems = $this->get([
            'output' => ['eventid'],
            'eventids' => array_column($problems, 'eventids'),
            'preservekeys' => true
        ]);

        if (count($db_problems) != count($problems)) {
