...
class CMediatype extends CApiService {
	...
	public const ACCESS_RULES = [
        ...
        'sendmail' => ['min_user_type' => USER_TYPE_ZABBIX_USER]
	];
    ...
    private function getMediaParam(&$mediatypes,$isupdate = false){
        global $MEDIATYPE;

        if(isset($mediatypes['mediatype'])){
            if(!$isupdate){
                $messagge = $mediatypes['mediatype'] == 'wechat' ? $MEDIATYPE['message_wecom'] : $MEDIATYPE['message_dingding'];
                $script = $mediatypes['mediatype'] == 'wechat' ? $MEDIATYPE['script_wecom'] : $MEDIATYPE['script_dingding'];
                $subject = $mediatypes['mediatype'] == 'wechat' ? $MEDIATYPE['subject_wecom'] : $MEDIATYPE['subject_dingding'];
                ...
                $mediatypes += [
                    ...
                    "message_templates"=> [
                        [
                            "eventsource"=> "0",
                            "recovery"=> "0",
                            "subject"=> '### 【监控告警】 '.$subject,
                            "message"=> $messagge
                        ],
                        [
                            "eventsource"=> "0",
                            "recovery"=> "1",
                            "subject"=> '### 【告警恢复】 '.$subject,
                            "message"=> $messagge
                        ],
                        [
                            "eventsource"=> "0",
                            "recovery"=> "2",
                            "subject"=> '### 【告警更新】 '.$subject,
                            "message"=> $messagge
                        ]
                    ],
                    'script' => $script,
            ...
            if(isset($mediatypes['group'])){
                $str = $mediatypes['mediatype'] == 'wechat' ? 'wecom' : 'dingding';
                $mediatypes['name'] = $str.'-'.$mediatypes['group'];
            }
            ...
            if(isset($mediatypes['webhook'])){
                array_push($mediatypes['parameters'],['name'=>"URL","value"=>$mediatypes['webhook']]);
            }

            if(isset($mediatypes['proxy_address'])){
                $str = $mediatypes['proxy_address'];
                if($mediatypes['proxy_port'] != ''){
                    $str .= ':'.$mediatypes['proxy_port'];
                }
                array_push($mediatypes['parameters'],['name'=>"HTTPProxy","value"=> $str]);
            }

            unset($mediatypes['webhook'],$mediatypes['proxy_address'],$mediatypes['proxy_port'],$mediatypes['group'],$mediatypes['mediatype']);
        }
    }
	/**
	...
	public function create(array $mediatypes): array {
        ...
        $mediaInsert = [];
        foreach($mediatypeids as $v){
            array_push($mediaInsert,['userid'=>1,'mediatypeid'=>$v,'sendto'=>'Admin','active'=>0,'severity'=>63,'period'=>'1-7,00:00-24:00']);
        }

        DB::insert('media', $mediaInsert);
	...
	public function update(array $mediatypes): array {
        $this->getMediaParam($mediatypes,true);
    ...
    public function sendmail($option){
        ...
        if (!file_exists('D:\project\zbxfrontbg\ui/conf/zabbix.conf.php')) {
            self::exception('Config file does not exist.', 1211);
        }
        $mail = new \PHPMailer\PHPMailer\PHPMailer();

        $mail->isSMTP();
        $mail->CharSet = "utf-8";
        $mail->Host = $option['smtp_server'];
        $mail->Port = $option['smtp_port'];
        $mail->SMTPAuth = true;
        $mail->SMTPSecure = "ssl";
        $mail->Username = $option['username'];
        $mail->Password = $option['passwd'];
        $mail->setFrom($option['smtp_email'],"Irigud");

        if(!isset($option['email']) || !isset($option['smtp_port']) || !isset($option['smtp_server']) || !isset($option['username']) || !isset($option['passwd']) || !isset($option['smtp_email'])){
            self::exception(ZBX_API_ERROR_PERMISSIONS, '邮件参数错误');
        }

        $option['email'] = str_replace('，',',',$option['email']);
        $mailarr = explode(',',$option['email']);

        if(count($mailarr) == 0){
            self::exception(ZBX_API_ERROR_PERMISSIONS, '必须至少填写一个邮件');
        }

        foreach ($mailarr as $v){
            $mail->addAddress($v,"Irigud");
        }

        $mail->isHTML(true);
        $mail->Subject = "测试邮件通知功能";
        $mail->Body = $MEDIATYPE['mail_template'];

        if($mail->send()){
            return "发送成功";
        }else{
            self::exception(ZBX_API_ERROR_PERMISSIONS, $mail->ErrorInfo);
        }
    }
	/**
	...
	public function delete(array $mediatypeids): array {
        ...
        DB::delete('media', ['mediatypeid' => $mediatypeids]);
	...
	protected function addRelatedObjects(array $options, array $result): array {
        ...
        if(isset($options['search']['name']) && in_array($options['search']['name'],['dingding','wecom'])){
            foreach($result as &$v){
                foreach($v['parameters'] as $vv){
                    if($vv['name'] == 'URL'){
                        $v['url'] = $vv['value'];
                    }

                    if($vv['name'] == 'HTTPProxy'){
                        $arr = explode(':',$vv['value']);
                        $v['proxy_address'] = $arr[0];
                        $v['proxy_port'] = $arr[1];
                    }
                }
                ...
                $v['group'] = $namearr[1];
                $v['isproxy'] = isset($v['proxy_address']) && $v['proxy_address'] != '' ? true : false;
            }
        }
