...
class CTrigger extends CTriggerGeneral {
	...
	public const ACCESS_RULES = [
        ...
        'enabletrigger' => ['min_user_type' => USER_TYPE_ZABBIX_ADMIN],
        'recover' => ['min_user_type' => USER_TYPE_ZABBIX_ADMIN],
	];
	...
	public function get(array $options = []) {
        ...
        $priority = ['未分类','信息','警告','一般严重','严重','灾难'];
        foreach($result as &$v){
            $v['total'] = API::getTotal('trigger',$options);
            $v['priority_str'] = $priority[$v['priority']];
            $v['isopen'] = $v['status'] == 0 ? true : false;
            $v['expressarr'] = $this->execExpression($v['expression']);
        }
    ...
    //拆分expression，返回expression数组
    private function execExpression($str){
        $ops = ['<>','>=','<=','>','=','<'];

        foreach($ops as $k => $v){
            if(strpos($str,$v) !== false){
                $temparr = explode($v,$str);
                return ['name'=>$temparr[0],'op'=>$k,'opvalue'=>$v,'value'=>$temparr[1]];
            }
        }
    }

    public function enableTrigger(array $triggers) {
        foreach($triggers as &$v){
            $v['status'] = $v['enable'] ? 0 : 1;
            unset($v['enable']);
        }
    ...
    public function recover(array $triggers) {
        $data = $this->get(['triggerids'=>$triggers,'output'=>['expression_default']]);

        foreach($data as &$v){
            $v['expression'] = $v['expression_default'];
        }

        $data = CMacrosResolverHelper::resolveTriggerExpressions($data,
            ['resolve_usermacros' => true, 'resolve_macros' => true, 'sources' => ['expression']]
        );

        $update = [];
        foreach($triggers as $k => &$v){
            foreach($data as $vv){
                if($vv['triggerid'] == $v && isset($vv['expression_default'])){
                    $update[$k]['triggerid'] = $vv['triggerid'];
                    $update[$k]['expression'] = $vv['expression'];
                }
            }
        }

        $this->validateUpdate($update, $db_triggers);

        $this->updateReal($update, $db_triggers);
