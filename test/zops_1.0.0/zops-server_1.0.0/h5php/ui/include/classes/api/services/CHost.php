...
class CHost extends CHostGeneral {
	...
	public function get($options = []) {
        ...
        if (!is_null($options['houseids'])) {
            zbx_value2array($options['houseids']);
            $sqlParts['where']['houseid'] = dbConditionInt('h.houseid', $options['houseids']);
        }

        if (!is_null($options['idcids'])) {
            zbx_value2array($options['idcids']);
            $sqlParts['where']['idcid'] = dbConditionInt('h.idcid', $options['idcids']);
        }

        if (!is_null($options['managerids'])) {
            zbx_value2array($options['managerids']);
            $sqlParts['where']['managerid'] = dbConditionInt('h.managerid', $options['managerids']);
        }

        if (!is_null($options['manufacturerids'])) {
            zbx_value2array($options['manufacturerids']);
            $sqlParts['where']['manufacturerid'] = dbConditionInt('h.manufacturerid', $options['manufacturerids']);
        }


        if (!is_null($options['inventory_typeids'])) {
            zbx_value2array($options['inventory_typeids']);
            $sqlParts['where']['inventory_typeid'] = dbConditionInt('h.inventory_typeid', $options['inventory_typeids']);
        }
        ...
        elseif (!is_null($options['discover_hosts'])) {
            $sqlParts['where']['status'] = 'h.status =2';
        }
        elseif(!is_null($options['all_hosts'])) {
            $sqlParts['where']['status'] = 'h.status IN ('.HOST_STATUS_MONITORED.','.HOST_STATUS_NOT_MONITORED.',2)';
        }
		else {
		...
		*/
        ...
        foreach($result as &$v){
            $total = $this->get(array_merge($options,['countOutput'=>true]));
            $v['total'] = $total ? $total : 0;
    ...
    public function doparam(&$data,$isupdate=false,$isdiscover=false){
        if(!isset($data['host']) && !$isupdate){
            $data['host'] = $data['name'];
        }
        ...
        $hosts = $this->get(['hostids'=>$data['hostid'],'selectInterfaces'=>'extend', 'all_hosts' => true]);
        $ip = isset($hosts[0]['interfaces'][0]) ? $hosts[0]['interfaces'][0]['ip'] : '127.0.0.1';

        //如果是更新操作并且没有更新的接口，则查询该设备的接口并赋值
        if($isupdate && !isset($data['interfaces'])){
            $data['interfaces'] = $hosts[0]['interfaces'];
        }

        //根据设备类型和接口协议的去匹配对应的模板id
        if((isset($data['groups']) || isset($data['groupid'])) && isset($data['interfaces'])){
            $groupids = [];
            ...
            if(isset($data['groupid'])){
                array_push($groupids,$data['groupid']);
                $data['groups'] = [['groupid'=>$data['groupid']]];
                unset($data['groupid']);
            }

            if(!isset($data['templates'])) $data['templates'] = [];
            $templateids = array_column($data['templates'],'templateid');
            $hostgroups = [];

            $groupres = DBselect('select * from hosts_groups where groupid in ('.implode(',',$groupids).')');
            while ($group = DBfetch($groupres)) {
                $hostgroups[] = $group;
            }

            $mainarr = [0,0,0,0];

            //赋予接口dns，main等初始值信息，否则报错
            foreach($data['interfaces'] as &$interface){
                if(!isset($interface['useip'])) $interface['useip'] = 1;
                if(!isset($interface['dns'])) $interface['dns'] = '';
                if(!isset($interface['details']['bulk']))$interface['details']['bulk'] = 1;
                if(!isset($interface['details']['max_repetitions'])) $interface['details']['max_repetitions'] = 10;

                if(!isset($interface['items'])) $interface['items'] = '';

                //如果没有ip，赋予默认ip
                if(!isset($interface['ip'])){
                    $interface['ip'] = $ip;
                    $interface['port'] = $interface['type'] == 1 ? '10050' : '161';
                }

                if(!isset($interface['main'])){
                    $interface['main'] = $mainarr[$interface['type']] == 0 ? 1 : 0;
                    if($mainarr[$interface['type']] == 0){
                        $mainarr[$interface['type']] = 1;
                    }
                }elseif(isset($interface['main']) && $interface['main'] == 1){
                    $mainarr[$interface['type']] = 1;
                }
            }

            //如果遍历完还有接口类型没有设置默认,有可能接口的main存在且全部类型都为0,这时候设置该类型第一个接口为默认，否则报错
            foreach($data['interfaces'] as &$interface){
                if($mainarr[$interface['type']] == 0){
                    $interface['main'] = 1;
                    $mainarr[$interface['type']] = 1;
            ...
            //根据设备类型id，接口的协议类型，在hosts_group查出对应的模板id
            foreach($hostgroups as $group){
                foreach($data['interfaces'] as &$interface){
                    if($group['checktype'] == $interface['type']){
                        if(!in_array($group['hostid'],$templateids)){
                            array_push($data['templates'],['templateid'=>$group['hostid']]);
                            array_push($templateids,$group['hostid']);
                        }
                    }
                }
            }
    ...
    public function discoverupdate($data){
        $hostids = array_column($data,'hostid');

        $res = $this->update($data,true);
        if($res){
            return $hostids;
        }else{
            self::exception(ZBX_API_ERROR_PARAMETERS, '系统错误');
        }
    }
	 ...
	 * @return array
	 */
	public function update($hosts,$isdiscover=false) {
        if(!isset($hosts['hostid'])){
            foreach($hosts as &$v){
               $this->doparam($v,true,$isdiscover);
            }
        }else{
            $this->doparam($hosts,true,$isdiscover);
        }
	...
	public function massUpdate($data) {
        ...
        if(isset($data['discover_hosts'])){
            $option['discover_hosts'] = $data['discover_hosts'];
            unset($data['discover_hosts']);
        }

		$db_hosts = $this->get($option);
	...
	protected function addRelatedObjects(array $options, array $result) {
        ...
        if ($options['selectIDC'] !== null) {
            ...
            $idcidArr = API::IDC()->get([
                'idcids' => $idcids,
                'output' => $options['selectIDC'],
                'countOutput' => $options['selectIDC'] == "count" ? true : false,
            ]);
        ...
        if ($options['selectManager'] !== null) {
            $managerids = array_column($result,'managerid');
            $manageridArr = API::User()->get([
                'userids' => $managerids,
                'output' => $options['selectManager'],
                'countOutput' => $options['selectManager'] == "count" ? true : false,
            ]);

            foreach ($result as $hostid => $host) {
                foreach($manageridArr as $manager){
                    if($host['managerid'] == $manager['userid']){
                        $result[$hostid]['manager'] = $manager;
                    }
                }
            }
        }

        if ($options['selectProblem'] !== null) {
            $problemArr = API::Problem()->get([
                'hostids' => $hostids,
                'output' => 'count'
            ]);

            foreach ($result as $hostid => $host) {
                $result[$hostid]['problem'] = 0;
                foreach($problemArr as $problem){
                    if($host['hostid'] == $problem['hostid']){
        ...
        if($options['selectModelType'] !== null){
            $models = array_column($result,'physical_model');

            $modelidArr = API::ModelType()->get([
                'output' => $options['selectModelType'],
                'countOutput' => $options['selectModelType'] == "count" ? true : false,
                'models'=>$models,
            ...
            foreach ($result as $hostid => $host) {
                foreach($modelidArr as $model){
                    if($host['physical_model'] == $model['physical_model']){
                        $result[$hostid]['modeltype'] = $model;
                    }
                }
            }
        }
            ...
            foreach ($result as $hostid => $host) {
                foreach($houseidArr as $house){
                    if($host['houseid'] == $house['houseid']){
                        $result[$hostid]['house'] = $house;
                    }
                }
            }
        }

        if ($options['selectInventoryType'] !== null) {
            $inventory_typeids = array_column($result,'inventory_typeid');
            $inventoryTypeArr = API::InventoryType()->get([
                'inventory_typeids' => $inventory_typeids,
                'output' => $options['selectInventoryType'],
                'countOutput' => $options['selectInventoryType'] == "count" ? true : false,
            ]);

            foreach ($result as $hostid => $host) {
                foreach($inventoryTypeArr as $inventoryType){
                    if($host['inventory_typeid'] == $inventoryType['inventory_typeid']){
                        $result[$hostid]['InventoryType'] = $inventoryType;
                    }
                }
            }
        }
			...
			if ($options['selectInterfaces'] != API_OUTPUT_COUNT) {
                ...
                foreach($result as &$v){
                    ...
                    foreach($v['interfaces'] as $kk => $vv){
                        ...
                        if(isset($v['protocal'][$type]) && $vv['available'] > $v['protocal'][$type] || !isset($v['protocal'][$type])){
                            $v['protocal'][$type] = $vv['available'];
                        }
                    }
                }
			}
...
//			}
	...
	protected function validateUpdate(array &$hosts, array &$db_hosts = null) {
		...
		foreach ($hosts as &$host) {
            ...
            if (array_key_exists('idcid', $host) || array_key_exists('houseid', $host)) {
                if($db_host['status'] != 0){
                    self::exception(ZBX_API_ERROR_PARAMETERS, _s('未纳管设备不能分配机房或数据中心'));
                }
            }
    ...
    public function exportinventory(array $options){
        $options = array_merge($options,['selectInventory'=>['type','hostid'],'selectManufacturer'=>['name','manufacturerid'],'selectHouse'=>['name','houseid'],'selectGroups'=>['name','groupid']]);
        ...
        $head = [
            ['head'=>'设备资产','key'=>'name'],
            ['head'=>'设备类型','key'=>['groups','name']],
            ['head'=>'厂商','key'=>['manufacturer','name']],
            ['head'=>'型号','key'=>['inventory','type']],
            ['head'=>'所属机房','key'=>['house','name']],
            ['head'=>'更新时间','key'=>'update_time']
        ];

        $res = $this->export($data,$head,'资产数据');
        return $res;
    }
