...
class CRole extends CApiService {
	...
	public function get(array $options = []) {
		$api_input_rules = ['type' => API_OBJECT, 'fields' => [
            ...
            'isfront' =>			    ['type' => API_BOOLEAN, 'default' => false],
			'startSearch' =>			['type' => API_FLAG, 'default' => false],
			'excludeSearch' =>			['type' => API_FLAG, 'default' => false],
			'searchWildcardsEnabled' =>	['type' => API_BOOLEAN, 'default' => false],

            'current' =>                ['type' => API_INT32, 'flags' => API_ALLOW_NULL, 'in' => '1:'.ZBX_MAX_INT32, 'default' => null],
			// output
			'output' =>					['type' => API_OUTPUT, 'in' => implode(',', ['roleid', 'name', 'type', 'readonly']), 'default' => API_OUTPUT_EXTEND],
			'countOutput' =>			['type' => API_FLAG, 'default' => false],
			'selectRules' =>			['type' => API_OUTPUT, 'flags' => API_ALLOW_NULL, 'in' => implode(',', ['ui','front', 'ui.default_access', 'services.read.mode', 'services.read.list', 'services.read.tag', 'services.write.mode', 'services.write.list', 'services.write.tag', 'modules', 'modules.default_access', 'api.access', 'api.mode', 'api', 'actions', 'actions.default_access']), 'default' => null],
        ...
        if($options['isfront']){
            $sql = 'SELECT role_ruleid,roleid,type,name,value_int,value_str,value_moduleid,value_serviceid FROM role_rule WHERE roleid in ('.implode(',',$options['roleids']).')';
        }

		$resource = DBselect($sql, $options['limit'],$options['current'] ? ($options['current'] - 1) * $options['limit'] : 0);
		...
		while ($row = DBfetch($resource)) {
            ...
            $db_roles[$row['roleid']]['total'] =API::getTotal('role',$options);
		}
	...
	private function validateCreate(array &$roles): void {
		$api_input_rules = ['type' => API_OBJECTS, 'flags' => API_NOT_EMPTY | API_NORMALIZE, 'uniq' => [['name']], 'fields' => [
			...
			'rules' =>			['type' => API_OBJECT, 'default' => [], 'fields' => [
                ...
                'front' =>						['type' => API_OBJECTS, 'flags' => API_NORMALIZE, 'fields' => [
                    'name' =>					['type' => API_STRING_UTF8, 'flags' => API_REQUIRED, 'length' => DB::getFieldLength('role_rule', 'value_str')],
                    'status' =>					['type' => API_INT32, 'in' => ZBX_ROLE_RULE_DISABLED.','.ZBX_ROLE_RULE_ENABLED, 'default' => ZBX_ROLE_RULE_ENABLED]
                ]],
	...
	private function validateUpdate(array &$roles, ?array &$db_roles): void {
		...
		$db_roles = $this->get([
			...
			'selectRules' => ['ui','front', 'ui.default_access', 'services.read.mode', 'services.read.list',
	...
	private function updateRules(array $roles, array $db_roles = null): void {
		$default_rules = [
			'ui' => [],
            'front' => [],
            'front.default_access' => ZBX_ROLE_RULE_ENABLED,
		...
		foreach ($roles as $roleid => $role) {
			...
			$rules[$roleid] = array_merge(
                $this->compileFrontRules((int) $type, $old_rules, $new_rules),
    ...
    private function compileFrontRules(int $type, array $old_rules, array $new_rules): array {
        $old_ui_rules = array_column($old_rules['front'], null, 'name');
        $new_ui_rules = array_column($new_rules['front'], null, 'name');

        $compiled_rules = [];

        foreach (CRoleHelper::getFrontElementsByUserType() as $ui_rule_name) {
            $ui_element = $ui_rule_name;
            ...
            elseif (array_key_exists($ui_element, $old_ui_rules)) {
                ...
                $ui_rule_status = $old_rules['front.default_access'];
            }

            if ($ui_rule_status != $new_rules['front.default_access']) {
                $compiled_rules[] = [
                    'name' => $ui_rule_name,
        ...
        $compiled_rules[] = [
            'name' => 'front.default_access',
            'type' => self::RULE_TYPE_INT32,
            'value' => $new_rules['front.default_access']
        ];
	...
	protected function addRelatedObjects(array $options, array $result): array {
		...
		if ($options['selectRules'] !== null) {
			if ($options['selectRules'] === API_OUTPUT_EXTEND) {
				$output = ['ui','front', 'ui.default_access', 'services.read.mode', 'services.read.list', 'services.read.tag',
			...
			foreach ($result as $roleid => &$role) {
                if($options['isfront']){
                    $role['rules'] = $this->getRelatedFrontRules($roles_rules[$roleid], $output);
                }else{
                    $role['rules'] = array_merge(
                        $this->getRelatedFrontRules($roles_rules[$roleid], $output),
    ...
    private function getRelatedFrontRules(array $rules): array {

        $front_default_access = array_key_exists('front.default_access', $rules)
            ? $rules['front.default_access']
            : (string) ZBX_ROLE_RULE_ENABLED;

        $result = [];

        $front = array_fill_keys(CRoleHelper::getFrontElementsByUserType(), $front_default_access);
        $front = array_intersect_key($rules, $front) + $front;

        $result['front'] = [];

        foreach ($front as $front_element => $status) {
            $result['front'][] = [
                'name' => $front_element,
                'status' => $status
            ];
