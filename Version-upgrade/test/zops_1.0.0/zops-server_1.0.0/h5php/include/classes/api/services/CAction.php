...
class CAction extends CApiService {
	...
	protected $sortColumns = ['actionid', 'name', 'status','clock'];
	...
	private const VALID_CONDITION_TYPES = [
		EVENT_SOURCE_TRIGGERS => [
			...
			CONDITION_TYPE_SUPPRESSED, CONDITION_TYPE_EVENT_TAG, CONDITION_TYPE_EVENT_TAG_VALUE,29,30,31
		],
	...
	public function get($options = []) {
		...
		while ($action = DBfetch($dbRes)) {
			if ($options['countOutput']) {
                ...
                $result[$action['actionid']]['total'] = API::getTotal('action',$options);
			}
		}
    ...
    //简化查询接口的前端数据
    private function simpleMessage(&$actions){
        $messageNameArr = ['1002'=>'邮件','1001'=>'短信','1004'=>'钉钉','1003'=>'企业微信'];
        $mediaArr = [];

        foreach($actions as $action){
            foreach($action['update_operations'] as $v){
                array_push($mediaArr,$v['opmessage']['mediatypeid']);
            }
        }

        $media = API::MediaType()->get(['mediatypeids'=>$mediaArr]);

        foreach($actions as &$action){
            foreach($action['update_operations'] as &$v){
                $v['opmessage']['type'] = 0;
                foreach($media as $vv){
                    if($vv['mediatypeid'] == $v['opmessage']['mediatypeid'] && (strpos($vv['name'],'wecom') !== false || strpos($vv['name'],'dingding') !== false)){
                        $v['opmessage']['type'] = strpos($vv['name'],'wecom') !== false ? 1 : 2;
                    }
                }
            }

//            if(isset($action['update_operations'])){
//                $message = [];
//                $arr = [];
//                foreach($action['update_operations'] as $k => $v){
//                    unset(
//                        $action['update_operations'][$k]['evaltype'],
//                        $action['update_operations'][$k]['operationtype'],
//                        $action['update_operations'][$k]['operationid'],
//                        $action['update_operations'][$k]['opmessage']['default_msg'],
//                        $action['update_operations'][$k]['opmessage']['subject'],
//                        $action['update_operations'][$k]['opmessage']['message']
//                    );
//                    $arr = $action['update_operations'][$k];
//
//                    //返回消息模板对应的名称
//                    $action['update_operations'][$k]['opmessage']['name'] = $messageNameArr[$action['update_operations'][$k]['opmessage']['mediatypeid']];
//                    array_push($message,$action['update_operations'][$k]['opmessage']);
//                    if($k > 0) unset($action['update_operations'][$k]);
//                }
//                $arr['opmessage'] = $message;
//                if(isset($action['update_operations'][0])) $action['update_operations'][0] = $arr;
//            }
            ...
            if(isset($action['filter'])){
                unset(
                    ...
                    $action['filter']['eval_formula']
                );

                foreach($action['filter']['conditions'] as $kk => $vv){
                    unset(
                        $action['filter']['conditions'][$kk]['operator'],
                        $action['filter']['conditions'][$kk]['value2'],
                        $action['filter']['conditions'][$kk]['formulaid']
                    );
                }
            }
        }
    ...
    //处理创建和编辑参数，简化前端传参
    private function doActionPrarams(&$actions,$isupdate = false){
        if(!isset($actions['eventsource']) && !$isupdate) $actions['eventsource'] = 0;

        if(isset($actions['media_group'])){
            foreach($actions['media_group'] as $v){
                $mediaGroup = ['opmessage_usr'=>[['userid'=>1]],'opmessage'=>['mediatypeid'=>$v]];
                if(!isset($actions['update_operations'])) $actions['update_operations'] = [];
                array_push($actions['update_operations'],$mediaGroup);
            }
            unset($actions['media_group']);
        }

        if(isset($actions['update_operations'])){
            foreach($actions['update_operations'] as $k => $v){
                if(!isset($v['operationtype'])) $actions['update_operations'][$k]['operationtype'] = $v['operationtype'] = 0;
//                if(is_array($v['opmessage'])){
//
//                    foreach($v['opmessage'] as $kk => $vv){
//                        if($kk == 0){
//                            $actions['update_operations'][$k]['opmessage'] = $vv;
//                        }else{
//                            $arr = $v;
//                            $arr['opmessage'] = $vv;
//                            array_push($actions['update_operations'],$arr);
//                        }
//                    }
//                }
            }
        ...
        if(isset($actions['filter'])){
            if(!isset($actions['filter']['evaltype'])) $actions['filter']['evaltype'] = 0;
        }
    }
	/**
	 * @param array $actions
	...
	public function update(array $actions): array {
        $this->doActionPrarams($actions,true);
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		if ($options['selectUpdateOperations'] !== null && $options['selectUpdateOperations'] != API_OUTPUT_COUNT) {
			...
			foreach ($update_operations as $update_operation) {
                $update_operation['grpcount'] = 0;
                $update_operation['usercount'] = 0;

                foreach($update_operation['opmessage_grp'] as $v){
                    $update_operation['grpcount']++;
                }

                foreach($update_operation['opmessage_usr'] as $v){
                    $update_operation['usercount']++;
                }
		...
		switch ($eventsource) {
			case EVENT_SOURCE_TRIGGERS:
				$value_rules = [
                    ...
                    ['if' => ['field' => 'conditiontype', 'in' => 29], 'type' => API_INT32],
                    ['if' => ['field' => 'conditiontype', 'in' => 30], 'type' => API_INT32],
                    ['if' => ['field' => 'conditiontype', 'in' => 31], 'type' => API_INT32],
	...
	private static function getOperationValidationRules(int $recovery, int $eventsource): array {
		...
		$all_opmessage_fields = [
            ...
            'opmessage_idc' =>	['type' => API_MULTIPLE, 'rules' => [
                ['if' => ['field' => 'operationtype', 'in' => OPERATION_TYPE_MESSAGE], 'type' => API_OBJECTS, 'uniq' => [['idcid']], 'fields' => [
                    'idcid' =>		['type' => API_ID, 'flags' => API_REQUIRED]
                ]],
                ['else' => true, 'type' => API_UNEXPECTED]
            ]],
            'opmessage_house' =>	['type' => API_MULTIPLE, 'rules' => [
                ['if' => ['field' => 'operationtype', 'in' => OPERATION_TYPE_MESSAGE], 'type' => API_OBJECTS, 'uniq' => [['houseid']], 'fields' => [
	...
	private function validateCreate(array &$actions): void {
		$api_input_rules = ['type' => API_OBJECTS, 'flags' => API_NOT_EMPTY | API_NORMALIZE, 'uniq' => [['name']], 'fields' => [
            ...
            'choose_type' =>			['type' => API_INT32, 'in' => implode(',', [0,1,2])],
        ...
        self::checkIDCPermissions($actions);
        self::checkHousePermissions($actions);
			...
			foreach (self::OPERATION_GROUPS as $recovery => $operation_group) {
				...
				foreach ($action[$operation_group] as &$operation) {
					...
					if ($operation['operationtype'] == OPERATION_TYPE_MESSAGE) {
                        ...
                        $has_idcs = array_key_exists('opmessage_idc', $operation) && $operation['opmessage_idc'];
                        $has_houses = array_key_exists('opmessage_house', $operation) && $operation['opmessage_house'];

						if (!$has_groups && !$has_users && !$has_idcs && !$has_houses) {
							self::exception(ZBX_API_ERROR_PARAMETERS,
    ...
    private static function checkIDCPermissions(array $actions): void {
        $idcs = [];
        ...
        $idcs = array_keys(array_flip($idcs));
    ...
    private static function checkHousePermissions(array $actions): void {
        $houses = [];
        ...
        $houses = array_keys(array_flip($houses));
