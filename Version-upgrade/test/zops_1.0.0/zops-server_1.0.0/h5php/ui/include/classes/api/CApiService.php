...
class CApiService {
    ...
    public function geturlWithHttp(){
        $pageURL = 'http';
        ...
        if ($_SERVER["SERVER_PORT"] != "80") {
            $pageURL .= $this->getIp() . ":" . $_SERVER["SERVER_PORT"];
        }else{
            $pageURL .= $this->getIp();
        }
        return $pageURL;
    ...
    public function queryParam($url,$data){
	...
	protected function customFetch($query, array $options) {
		if ($this->requiresPostSqlFiltering($options)) {
			$offset = $options['current'] ? ($options['current'] - 1) * $options['limit'] : 0;
    ...
    protected function getLetter($num){
        $times = floor(($num - 1) / 25);

        return $times >= 1 ? chr(64 + $times) . chr($num - 25 * $times + 64) : chr($num + 65);
    }
    ...
    //上传图片，转化base64为图片并保存
    protected function base64toimg($base64_image_content){
        $root_path = __DIR__.'/../../../';
        ...
        if(preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content,$result)){
            ...
            if (file_put_contents($root_path.$new_file, base64_decode(str_replace($result[1], '', $base64_image_content)))){
                return $this->getpageurl().'/'.$new_file;
            }else{
                return false;
            }
        }else{
    ...
    //excel导出
    protected function export($list,$head,$filename)
    {
        //1.从数据库中取出数据
        ...
        foreach($head as $k => $v){
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($this->getLetter($k).'1',$v['head']);

            $objPHPExcel->setActiveSheetIndex(0)->getStyle($this->getLetter($k))->getAlignment()
                ->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT);

            //设置单元格宽度
            $objPHPExcel->setActiveSheetIndex(0)->getColumnDimension($this->getLetter($k))->setWidth(20);
        }
        ...
        foreach($list as $k => $v) {
            foreach ($head as $kk => $vv) {
                //是否在原数组是子数组数据
                if(is_array($vv['key'])){
                    $value = isset($v[$vv["key"][0]][0]) ? $v[$vv["key"][0]][0][$vv['key'][1]] : $v[$vv['key'][0]][$vv['key'][1]];
                }else{
                    $value = $v[$vv['key']];
                }

                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($this->getLetter($kk) . ($k + 2),$value);
            }
        }
        ...
        $files = $root_path. $filepath.'/'. $filename;
        $objWriter->save($files);//保存文件

        return $this->getpageurl() . $filepath .'/'.  $filename;
    }
