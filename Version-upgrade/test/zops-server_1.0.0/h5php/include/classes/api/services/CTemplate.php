...
class CTemplate extends CHostGeneral {
	...
	public function get($options = []) {
		...
		while ($template = DBfetch($res)) {
			if ($options['countOutput']) {
                ...
                $result[$template['templateid']]['templatename'] = '';
                $result[$template['templateid']]['total'] = API::getTotal('template',$options);;
			}
		}
	...
	protected function addRelatedObjects(array $options, array $result) {
        ...
        $hostgroup = API::HostGroup()->get(['hostids'=>$templateids]);

        foreach($hostgroup as $v){
            $result[$v['hostid']]['templatename'] = $v['name'];
            $result[$v['hostid']]['image'] = $this->geturlWithHttp() . $v['image'];
        }
