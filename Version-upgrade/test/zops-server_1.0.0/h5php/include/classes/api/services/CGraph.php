...
class CGraph extends CGraphGeneral {
    ...
    private function  validateTime($from, $to) {
        if ($from === null || $to === null) {
            return ['code'=>false,'msg'=>'invalid params'];
        }
        ...
        if ($period < ZBX_MIN_PERIOD) {
            return(['code'=>false,'msg'=>_n('Minimum time period to display is %1$s minute.',
                'Minimum time period to display is %1$s minutes.', (int) (ZBX_MIN_PERIOD / SEC_PER_MIN)
            )]);
        }
        elseif ($period > $max_period) {
            return(['code'=>false,'msg'=>_n('Maximum time period to display is %1$s day.',
    ...
    public function getGraphById($option){
       $validres = $this->validateTime($option['from'],$option['to']);

       if(!$validres['code']) return $validres['msg'];
        ...
        return(['data'=>$res['data'],'timex'=>$res['timex'],'itemGroup'=>$dbGraph]);
    }
