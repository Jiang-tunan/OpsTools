...
class CItem extends CItemGeneral {
	...
	public function get($options = []) {
		...
		while ($item = DBfetch($res)) {
			...
			if (!$options['countOutput']) {
                ...
                $result[$item['itemid']]['total'] = API::getTotal('item',$options);
				continue;
			}
		...
		foreach ($result as &$item) {
            ...
            $item['isopen'] = $item['status'] == 0 ? true : false;
            $item['type_str'] = item_type2str($item['type']);
		}
		unset($item);
    ...
    //获取设备的接口监控项列表
    public function interfacelist(array $items) {
        if($items['type'] == 0){
            $tag = 'interface';
        }else{
            $tag = 'description';
       ...
       $data = $this->get(array_merge($items,["searchByAny"=>true,'search'=>['name'=>'Speed'],'tags'=>[['tag'=>$tag]]]));

       $search = $items;
       unset($search['limit'],$search['current']);
       $statusdata = $this->get(array_merge($search,["searchByAny"=>true,'search'=>['name'=>'Operational'],'tags'=>[['tag'=>$tag]]]));
       $itemids = array_merge(array_column($data,'itemid'),array_column($statusdata,'itemid'));
       $history = API::History()->get(['itemids'=>$itemids,'history'=>3,'time_from'=>time() - 86400,'time_till'=>time(),'sortorder'=>'ASC','sortfield'=>'clock']);
       ...
       foreach($data as $v){
           ...
           if(!isset($res[$namearr[0]]) && strpos($namearr[1],'Speed') != false){
               $res[$namearr[0]] = ['name'=>$v['name'],'value'=>0,'status'=>0,'itemid'=>$v['itemid']];

               foreach($history as $vv){
                   if($vv['itemid'] == $v['itemid']){
                       $res[$namearr[0]]['value'] = round($vv['value'] / 1000000,2);
                   }

                   //处理状态
                   foreach($statusdata as $vvv){
                       if($vv['itemid'] == $vvv['itemid']){
                           $statusnamearr = explode(':',$vvv['name']);
                           if(isset($res[$statusnamearr[0]])){
                               $res[$statusnamearr[0]]['status'] = $vv['value'];
                           }
    ...
    //监控项开启,关闭
    public function enable(array $items) {
        $items['status'] = $items['enable'] ? 0 : 1;
        unset($items['enable']);
